...
export function WorkChat() {
  ...
  const handleStageChoiceClick = (choice: string) => {
    if (!choice.trim()) {
      return;
    }
          ...
          <div className="flex flex-col gap-2 pt-1">
            {stageMetadata.choices.map((choice) => (
              <button
                key={choice}
                type="button"
                onClick={() => handleStageChoiceClick(choice)}
                disabled={loading}
                className={clsx(
                  'rounded-2xl border border-white/10 px-3 py-2 text-left text-xs font-semibold transition sm:text-sm',
                  loading
                    ? 'cursor-not-allowed bg-white/10 text-blue-100/60'
                    : 'bg-white/10 text-blue-100 hover:border-blue-200/40 hover:bg-white/20 focus:border-blue-200/60 focus:outline-none focus:ring-2 focus:ring-blue-300/40',
                )}
              >
                {choice}
              </button>
